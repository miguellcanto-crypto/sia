...
import { LayoutGrid, Landmark, Wallet, X, LogOut } from 'lucide-react';
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            fetch('/api/cash/registers')
                ...
                .finally(() => setIsFetchingRegisters(false));
        }
    }, [isOpen]);
                    ...
                    >
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 gap-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                                    {isFetchingRegisters ? (
                                        ...
                                        <div className="p-8 text-center bg-slate-50 dark:bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700">
                                            <p className="text-sm text-slate-500 italic">No hay cajas disponibles configuradas.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="space-y-3 pt-2">
                                ...
                                <button
                                    onClick={() => signOut()}
                                    className="w-full py-4 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 rounded-2xl font-semibold text-sm transition-all flex items-center justify-center gap-2"
                                >
